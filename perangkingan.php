...
<form method="post">
	...
	<div class="input-control select full-size">
			<select name="alt">
			<option value="<?php echo isset($_GET['nama']) ? $_GET['nama'] : ''; ?>"><?php echo isset($_GET['nama']) ? $_GET['nama'] : ''; ?></option>
			<?php
			$stmt3 = $db->prepare("select * from smart_siswa");
			$stmt3->execute();
			while ($row3 = $stmt3->fetch()) {
			?>
				<option value="<?php echo $row3['id_siswa'] ?>"><?php echo $row3['nama_siswa'] ?></option>
			<?php
			}
			?>
		</select>
